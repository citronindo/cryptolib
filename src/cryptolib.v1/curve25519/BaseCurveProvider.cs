...
namespace citronindo.crypto.curve25519
{
    ...
    /// </summary>
    public abstract class BaseCurveProvider : CurveProvider
    {
        private ISha512? sha512provider;
        private SecureRandomProvider? secureRandomProvider;

        public BaseCurveProvider()
        {
            sha512provider = null;
            secureRandomProvider = null;
        }

        protected BaseCurveProvider(ISha512 sha512provider,
        ...
        public override byte[] generatePublicKey(byte[] privateKey)
        {
            ...
            Keygen.Curve_keygen(publicKey, privateKey);
        ...
        {
            byte[] random = getRandom(CurveProvider.PRIVATE_KEY_LEN);
        ...
        {
            return Curve_sigs.Curve_verify(sha512provider, signature, publicKey, message, message.Length) == 0;
        }
